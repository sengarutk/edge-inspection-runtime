...
As shown in Table~\ref{tab:ablation_results} and Fig.~\ref{fig:pareto_queue}(a), \texttt{FULL\_POLICY} achieves \GlitchSuppressionFull{} false-alarm suppression on transient glitches and reduces false-alarm rates by \SustainedSuppressionFull{} in sustained defects. The resulting workload remains below the 60 reviews/hour cognitive threshold, while detection delay is bounded at \SustainedMedianDelay{} frames.

Queueing dynamics in Fig.~\ref{fig:pareto_queue}(b) show that \texttt{FULL\_POLICY} bounds operator utilization to $\rho=0.20$ at an arrival rate of 12 reviews/hour, corresponding to an expected triage wait of $W_q=0.25$ minutes. In contrast, unmitigated baseline alerts saturate the triage station when $\rho\geq1$.
